...
#' @examples
#' stackModel = LDL_ML_Main_StackingAlgorithm("data.csv",0.8,ReportMultiPlot = TRUE,ComparisonPlot = TRUE)
#' @export
LDL_ML_Main_StackingAlgorithm<-function(DataCSV, partition, ReportMultiPlot = TRUE, ComparisonPlot = TRUE){
  ...
  stackAlgPredictions = LDL_predict(stackAlgModel,validationData)
