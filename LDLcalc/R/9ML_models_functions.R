...
#' @examples
#' modelPrediction=LDL_predict(model,c(data.table(CHOL=170,HDL=35,TG=174)))
#' modelPrediction=LDL_predict(model,Data)
#' modelPrediction=LDL_predict(stackModel,Data)
#' @export
LDL_predict<-function(model,data){
...
#' @examples
#' model = LDL_ML_Main("data.csv",0.7,"lm",ReportMultiPlot=FALSE)
#' @export
LDL_ML_Main<-function(DataCSV, partition, MLmethod, ReportMultiPlot = TRUE){
  ...
  if (!(MLmethod%in%MLmethodList)){
    print("ML method entered does not exist. You entered: ")
    print(MLmethod)
    return (-3)
  }
  ...
  predictions = LDL_predict(model,validationData)
